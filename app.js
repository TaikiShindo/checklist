...
const GEMINI_MODELS = [
  ...
  { value: 'gemini-3.1-pro-preview', label: 'Gemini 3.1 Pro（プレビュー）' },
  { value: 'gemini-2.5-flash',       label: 'Gemini 2.5 Flash（前世代）' },
...
function loadModel() {
  return localStorage.getItem(API_MODEL_STORAGE) || 'gemini-3.5-flash';
}
...
function migrateDeprecatedModel() {
  ...
  if (saved && DEPRECATED_MODELS.includes(saved)) {
    console.warn(`[移行] 廃止されたモデル "${saved}" を "gemini-3.5-flash" に自動変更しました。`);
    localStorage.setItem(API_MODEL_STORAGE, 'gemini-3.5-flash');
  }
}
